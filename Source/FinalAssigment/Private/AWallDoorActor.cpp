...
// Sets default values
AAWallDoorActor::AAWallDoorActor()
{
    ...
    DoorWidth = 90.0f;
    DoorHeight = 200.0f;
    DoorLocation = FVector(0.0f, 0.0f, 0.0f);
...
void AAWallDoorActor::SetDoorLocation(float X)
{
	DoorLocation.X = X;
...
void AAWallDoorActor::AttachDoor(UStaticMesh*& DoorStaticMesh)
{
	if (DoorStaticMesh)
	{
...
void AAWallDoorActor::BeginPlay()
{
	Super::BeginPlay();
	
}

void AAWallDoorActor::SetParentActor(AActor* parentActor)
{
    if(parentActor)
    {
...
AActor* AAWallDoorActor::GetParentActor()
{
    return ParentActor;
}

// Called every frame
void AAWallDoorActor::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void AAWallDoorActor::OnConstruction(const FTransform& Transform)
{
...
void AAWallDoorActor::AddNormals(TArray<FVector>& Normals)
{
    Normals = {
...
void AAWallDoorActor::AddUVs(TArray<FVector2D>& UVs)
{
...
void AAWallDoorActor::CreateMesh()
{
    float L = 100;  
    float W = 20;  
    float H = 400;  
    ...
    DoorWidth = FMath::Min(DoorWidth, H);
    DoorHeight = FMath::Min(DoorHeight, H - 10);

    if (DoorLeft < -L / 2 || DoorRight > L / 2 || DoorHeight > H)
    {
